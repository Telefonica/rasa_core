...
def create_argument_parser():
    ...
    parser.add_argument('--mlflow', required=False, default='True',
                        help="choose if the data can be recorded in MLFLOW")

    parser.add_argument('--tracker', required=True, default='http://localhost:5000',
                        help="URI for the MLFLOW TRACKER SERVER")
...
def main():
    ...
    if cmdline_args.mode == "crossvalidation":
        ...
        result = run_evaluation(cmdline_args.data,
                                cmdline_args.model,
                                cmdline_args.errors,
        ...
        if (cmdline_args.mlflow=='True'):
            ...
            r_entity = result['entity_evaluation']['ner_crf']['report'].replace('\n\n', '\n').replace('avg / total','avg/total')
            extract_report(r_entity, 'entity_report.csv')

            r_intent = result['intent_evaluation']['report'].replace('\n\n', '\n').replace('avg / total', 'avg/total')
            extract_report(r_intent, 'intent_report.csv')
