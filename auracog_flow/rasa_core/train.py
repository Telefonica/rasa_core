...
def get_no_of_stories(story_file, domain):
    ...
    #FIXING BUG_TO_CALCULATE_NUMBER_OF_STORIES
    #******************************************
    ...
    for i in range(len(stories)):
        line=str(stories[i])
        a=str(stories[i]).find("block_name='")
        name=str(stories[i])[a+len("block_name='"):str(stories[i]).find("',")]
        if name in num_app:
            num_app[name] = num_app[name]+1
        else:
            num_app[name]=1

    # Go thorugh the dic to measure the number of rows to sub

    iterac=num_app.values()
    for i in iterac:
        if i > 0:
            cont=cont+1
        else:
    ...
    print(len(stories))
    print (len(stories)-cont)
    #return len(stories)
    return (len(stories)-cont)
