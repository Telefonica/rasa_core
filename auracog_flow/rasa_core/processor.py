...
from auracog_flow.rasa_core.tracker_store import TrackerStore
from auracog_flow.rasa_core.trackers import DialogueStateTracker, EventVerbosity
from rasa_core.utils import EndpointConfig

from auracog_flow.rasa_core.normalizers import UserMessageNormalizer
from .constants import INTENT_UNKNOWN_RECEIVED, INTENT_UNKNOWN
...
class MessageProcessor(object):
    ...
    def predict_next_action(self,
        ...
        # Extra logging for probability prediction
        _predicted_probs = {self.domain.action_for_index(i, self.action_endpoint).name(): probabilities[i] for i in range(len(probabilities))}
        logger.debug("Predicted actions probabilities: {}".format(_predicted_probs))
        _min_prob_difference = 1
        _secondary_index = -1
        for i in range(len(probabilities)):
            _diff = probabilities[max_index] - probabilities[i]
            if _diff > 0:
                if _diff < _min_prob_difference:
                    _min_prob_difference = _diff
                    _secondary_index = i
        if _secondary_index >= 0:
            logger.debug("Secondary action prediction: {} with delta prob {}".format(self.domain.action_for_index(_secondary_index, self.action_endpoint).name(), _min_prob_difference))
        #
    ...
    def _handle_message_with_tracker(self,
        ...
        # Aura specific
        normalized_parse_data = UserMessageNormalizer.normalize_parse_data(parse_data, self.domain)
        logger.debug("Received UserMessage {}".format(message))
        logger.debug("Received UserMessage (normalized parse_data) {}".format(normalized_parse_data))

        # don't ever directly mutate the tracker
        # - instead pass its events to log
        _event_user_uttered = UserUttered(message.text, normalized_parse_data["intent"],
                                   normalized_parse_data["entities"], normalized_parse_data,
                                   input_channel=message.input_channel,
                                   message_id=message.message_id)
        tracker.update(_event_user_uttered)

        # Aura specific:
        # Process automatic slots reset, before slots are set
#        MessageProcessor._log_slots(tracker)  # Debug
        tracker.process_reset_slots_delta(_event_user_uttered)
#        MessageProcessor._log_slots(tracker)  # Debug

        # store all entities as slots
        for e in self.domain.slots_for_entities(normalized_parse_data["entities"]):
            tracker.update(e)

        if self._is_intent_unknown_received(normalized_parse_data):
            tracker.update(SlotSet(INTENT_UNKNOWN_RECEIVED, True))

        # Aura specific:
        # Process automatic context update, after slots are set
#        MessageProcessor._log_slots(tracker)  # Debug
        tracker.process_context_update(_event_user_uttered)
#        MessageProcessor._log_slots(tracker)  # Debug

        logger.debug("Logged UserUtterance - "
                     "tracker now has {} events".format(len(tracker.events)))

    def _is_intent_unknown_received(self, parse_data):
        _intent = parse_data.get("intent", None)
        if _intent is not None:
            _intent_name = _intent.get("name", None)
            return _intent_name == INTENT_UNKNOWN
        return False
    ...
    def _run_action(self, action, tracker, dispatcher, policy=None,
        ...
        # Aura specific:
        # Process automatic context update, after action is run
        tracker.process_context_update_after_action(action)
