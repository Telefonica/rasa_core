...
import mlflow
from auracog_flow.Metrics.eval_func import keep_model, extract_report
...
from auracog_flow.rasa_core.agent import Agent
...
from auracog_flow.rasa_core.interpreter import LuisHttpInterpreter
...
def create_argument_parser():
    ...
    # Add Aura arguments
    add_aura_args_to_parser(parent_parser)
...
def add_aura_args_to_parser(parser):
    # Aura NLU options
    parser.add_argument(
            '--nlu_server_type',
            default="local",
            type=str,
            help="NLU server type: local|luis")
    parser.add_argument(
            '--luis_endpoint',
            default=None,
            type=str,
            help="LUIS server endpoint (e.g. http://localhost:5000")
    parser.add_argument(
            '--project',
            default=None,
            type=str,
            help="Rasa NLU project name. Only used if a Rasa based LUIS interface is used. If LUIS is actually used, "
                 "this parameter should not be used.")
    parser.add_argument(
            '--model',
            default=None,
            type=str,
            help="Rasa NLU model name. Only used if a Rasa based LUIS interface is used. If LUIS is actually used, "
                 "this parameter should not be used.")
    return parser
...
def _in_training_data_fraction(action_list):
    ...
    for a in action_list:
        if a["policy"] is None:
            logger.warn("policy is None in {}".format(a))
            continue
        if not SimplePolicyEnsemble.is_not_memo_policy(a["policy"]):
            in_training_data.append(a["action"])
...
def log_evaluation_table(golds, name,
    ...
    if (cmdline_arguments.mlflow == 'True'):
        mlflow.log_metric("{}_correct".format(name), (int(len(golds) * accuracy)))
        mlflow.log_metric('{}_total'.format(name), len(golds))
        mlflow.log_metric('{}_precision'.format(name), precision)
        mlflow.log_metric('{}_accuracy'.format(name), accuracy)
        mlflow.log_metric('{}_f1'.format(name), f1)
...
def plot_story_evaluation(test_y, predictions,
    ...
    fig.savefig(os.path.join(out_directory, "story_confmat.png"),
                bbox_inches='tight')
...
def plot_curve(output, no_stories):
    ...
    plt.savefig(os.path.join(output, 'model_comparison_graph.png'),
                format='png')
    #plt.show()
...
if __name__ == '__main__':
    ...
    if cmdline_arguments.mode == 'default':
        ...
        # Select Interpreter according to comand line options
        if cmdline_arguments.nlu_server_type == "local":
            _interpreter = NaturalLanguageInterpreter.create(cmdline_arguments.nlu,
                                                             _endpoints.nlu)
        elif cmdline_arguments.nlu_server_type == "luis":
            _interpreter = LuisHttpInterpreter(cmdline_arguments.luis_endpoint,
                                               cmdline_arguments.project,
                                               cmdline_arguments.model)
        else:
            raise RuntimeError("Wrong nlu_server_type: {}. Valid values are local|luis".format(
                cmdline_arguments.luis_endpoint))
        ...
        report=run_story_evaluation(stories,
                             _agent,
        ...
        if (cmdline_arguments.mlflow == 'True'):
            ...
            r_metrics = report['report'].replace('\n\n', '\n').replace('avg / total', 'avg/total')
            extract_report(r_metrics, 'core_report.csv')

            # Logging the artifacts #
            # *************************

            #mlflow.log_artifact('results/story_confmat.png', 'images')
            #mlflow.log_artifact('results/failed_stories.md', 'reports')

            mlflow.log_artifact(cmdline_arguments.output+'/story_confmat.png', 'images')
            mlflow.log_artifact(cmdline_arguments.output+'/failed_stories.md', 'reports')
        ...
        if (cmdline_arguments.mlflow == 'True'):
            mlflow.log_artifact('comparison_results/model_comparison_graph.png', 'images')
            mlflow.log_param('EVALUATION', 'POLICIES')
